...
class Scheduler:
    ...
    def refresh_slot_jobs(self):
        try:
            ...
            slots = current_app.config['db'].get(
                ["slot_id", 'day', 'time_start'], 0, "slots")
            for slot in slots:
                ...
                t = datetime.datetime.combine(
                    date=datetime.date(30, 1, 1), time=time_start)
                t = t - datetime.timedelta(minutes=30)
                notif_time = t.time()

                # print(day, notif_time)
                current_app.apscheduler.add_job(func=self.send_slot_notification, trigger='cron', day_of_week=day, args=[
                                                slot_id, day, time_start], id=str(slot_id), hour=notif_time.hour, minute=notif_time.minute, timezone=timezone('Asia/Kolkata'))
            ...
            if(volunteers != -44):
                for volunteer in volunteers:
                    ...
                    self.send_notification(
                        volunteer["name"], volunteer["fcm_token"], time_start)
        except Exception as e:
            print(e)
    ...
    def send_notification(self, volunteer_name, volunteer_fcm_token, slot_time: datetime.time):
        try:
            ...
            response = requests.request(
                "POST", self.fcm_url, headers=headers, data=payload)
