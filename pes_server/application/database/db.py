...
vs.pes_id = v.pes_id;"""
    ...
    def create_volunteer(self, volunteer):
        ...
        args = [AsIs('volunteers'), str(volunteer.PES_ID), str(volunteer.Password), str(volunteer.Name), str(volunteer.Phone), str(volunteer.Profession), str(volunteer.Email), str(volunteer.address), str(volunteer.Pathshaala), str(volunteer.Status), str(volunteer.joining_date)]
        # try:
        cursor = self.connection.cursor()
    ...
    def get_volunteer(self, pesId):
        ...
        result = cursor.fetchone()
        if(result):
            user_data = {"Name": result[2], "Phone": result[3], "Profession": result[4], "Email": result[5], "Pathshaala": result[6], "Address": result[7]}
            #print(user_data)
            return user_data
        else:
            return -1
    ...
    def volunteer_slots(self,pesId):
        ...
        """
        cmd='select s.*, sb.remarks from slots s,volunteer_slots vs, syllabus sb where vs.pes_id =%s'+"""and vs.slot_id = s.slot_id and sb.batch = s.batch order by day;""" 
        cursor = self.connection.cursor()
        args = [str(pesId)]
        cursor.execute(cmd, args)
        outputParams = ['slot_id','pathshaala','batch','day','description','time_start','time_end', 'remarks', 'batch_remarks']
        tuples = cursor.fetchall()
        result = []
        for i in tuples:
            dic = {}
    ...
    def all_slots(self,):
        ...
        cursor = self.connection.cursor()
        cursor.execute(cmd)
        outputParams = ['pathshaala','day','time_start','time_end', 'slot_id', 'remarks','batch', 'batch_remarks']
        tuples = cursor.fetchall()
        result = []
        for i in tuples:
            dic = {}
    ...
    def insert_application(self,inputParams):
        ...
        """
        check_key = self.get(["name", "phone"], {"phone": str(inputParams['phone'])}, "applications")
        if (len(check_key) >= 1):
            return -20
        args = [AsIs('applications'), str(inputParams['name']), str(inputParams['phone']), str(inputParams['profession']), str(inputParams['email']), str(inputParams['address']), str(inputParams['pathshaala']), str(inputParams['text1']), str(inputParams['text2'])]
    ...
    def remove_application(self, phone):
        check_key = self.get(["name", "phone"], {"phone": str(phone)}, "applications")
    ...
    def remove_leaving_application(self, pes_id):  
        ...
        """
        check_key = self.get(["pes_id"], {"pes_id": pes_id}, "leaving_pehchaan")
    ...
    def add_slot(self, data):
        ...
        """
        args = [AsIs('slots'), str(data['pathshaala']), str(data['batch']), str(data['day']), "No description", str(data['time_start']), str(data['time_end']), " "]
    ...
    def edit_slot(self, data):
        ...
        """
        check_slot_id = self.get(["slot_id", "pathshaala"], {"slot_id": data['slot_id']}, "slots")
        if (len(check_slot_id) == 0):
            return -20
        args = [AsIs('slots'), str(data['pathshaala']), str(data['batch']), str(data['day']), str(data['description']), str(data['time_start']), str(data['time_end']), str(data['remarks']), str(data['slot_id'])]
        cmd = "UPDATE %s SET "
        cmd += "pathshaala = %s, "
        cmd += "batch = %s, "
        cmd += "day = %s, "
    ...
    def delete_slot(self, data):
        ...
        for slot in data['slot_ids']:
            check_slot_id = self.get(["slot_id", "pathshaala"], {"slot_id": slot}, "slots")
            if (len(check_slot_id) == 0):
                lst[0].append(slot)
            else:
    ...
    def add_vol(self, data):
        ...
        """
        #try:
        check_pes_id = self.get(["pes_id", "name"], {"pes_id": data['pes_id']}, "volunteers")
    ...
    def edit_vol(self, data):
        check_slot_id = self.get(["pes_id", "name"], {"pes_id": data['pes_id']}, "volunteers")
        if (len(check_slot_id) == 0): # doesnt exist
            return -20
        args = [AsIs('volunteers'), str(data['pes_id']), str(data['password']), str(data['name']), str(data['phone']), str(data['profession']), str(data['email']), str(data['pathshaala']), str(data['status']), str(data['address']), str(data['pes_id'])]
        cmd = "UPDATE %s SET "
        cmd += "pes_id = %s, "
        cmd += "password = %s, "
        cmd += "name = %s, "
    ...
    def all_vols_attendance(self, count):
        ...
        ;"""
        outputParams = ["pesId", "totalSlots","name","pathshaala", "profession", "slotsAttended"]
        cursor = self.connection.cursor()
        args = [AsIs(str(count))]
        cursor.execute(cmd, args)
        tuples = cursor.fetchall()
    ...
    def edit_batch(self, data):
        ...
        cmd += " WHERE batch = %s;"
        cursor = self.connection.cursor()
        args = [AsIs('syllabus'), str(data['syllabus']), str(data['remarks']), str(data['batch'])]
        cursor.execute(cmd, (args))
        return 1
    ...
    def view_admin(self):
        ...
        for admin in get_admin_id:
            get_admin_data = self.get(["name", "phone", "profession", "email", "pathshaala", "status", "address", "joining_date"], {"pes_id": admin["admin_id"]}, "volunteers")
    ...
    def add_notif(self,title,data):
        ...
        cursor=self.connection.cursor()
        args = [AsIs('notifications'), title, data, str(datetime.now(timezone('Asia/Kolkata')).strftime("%m-%d-%Y, %H:%M:%S"))]
        cursor.execute(cmd, args)
        return 1
    ...
    def get_vol_info(self, lst):
        ans = []
        for vol in lst:
            name_profession = self.get(["name", "profession"], {"pes_id": vol["pes_id"]}, "volunteers")
    ...
    @handle_error(-44)
    def vol_count(self, today_date, first_day): # To check how many vols have joined since the beginning of the year
